...
def generate_logger():
    ...
    """
    try:
        sub_path = 'analyst_l1_queueing_agent_module_logs/analyst_l1_queueing_agent_module_logs_'
        file_name = check_file_or_create(sub_path, ".log")
        if file_name:
            try:
                log_process_activities(
                    'analyst_l1_queueing_agent_module_logs', file_name)
                logger = logging.getLogger(
                    'analyst_l1_queueing_agent_module_logs')
            except Exception as e:
                log_process_activities(
                    'analyst_l1_queueing_agent_module_logs', file_name)
                logger = logging.getLogger(
...
ysoc_secrets = {'ysoc_pcap_analysis_script': {
                "elastic_endpoint": "https://ELstic_URL_:9243",
